...
const AddComment = ({ postId, comments = 0 }: Props) => {
	const { openModal } = useModalActions();
	return (
		<button
			className="group flex items-center gap-x-1"
			onClick={() => openModal<ModalType.Comments>(ModalType.Comments, { postId })}
		>
			<motion.svg
				width="24"
				height="24"
			...
			>
				<path
					className="fill-[#5B7083] group-hover:fill-black"
			...
			<span className="text-xs text-[#5B7083] group-hover:text-black sm:text-sm">{comments}</span>
		</button>
	);
};
